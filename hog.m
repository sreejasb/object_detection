...
for i = 1:nori
    ...
    B = (mag > thresh) & ((ori >= orientation) & (ori < orientation + pi/9));
    ...
    [blocksize, numblocks] = size(chblock);
    for y = 1:numblocks
        sums(y) = sum(chblock(:,y));
    end    
    ohist(:,:,i) = reshape(sums, [h2,w2]);
    orientation = orientation + pi/9;
end
...
for i = 1:h2
    for y = 1:w2
        clear sum;
        summation = sum(ohist(i,y,:));
        if summation==0
            continue;
        end
        ohist(i,y,:) = ohist(i,y,:) / summation;
    end
end